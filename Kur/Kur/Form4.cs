...
namespace Kur
{
    public partial class person : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < personalDataGridView.ColumnCount - 1; i++)
            {
                for (int j = 0; j < personalDataGridView.RowCount - 1; j++)
                {
                    personalDataGridView[i, j].Style.BackColor = Color.White;
                    personalDataGridView[i, j].Style.ForeColor = Color.Black;
                }
            }
            ...
            for (int i = 0; i < personalDataGridView.ColumnCount - 1; i++)
            {
                for (int j = 0; j < personalDataGridView.RowCount - 1; j++)
                {
                    if (personalDataGridView[i, j].Value.ToString().IndexOf(textBox1.Text) != -1)
                    {
                        personalDataGridView[i, j].Style.BackColor = Color.AliceBlue;
                        personalDataGridView[i, j].Style.ForeColor = Color.Blue;
                    }
                }
            }
        }
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            if (radioButton1.Checked) personalDataGridView.Sort(COL, System.ComponentModel.ListSortDirection.Ascending);
            else personalDataGridView.Sort(COL, System.ComponentModel.ListSortDirection.Descending);
        }

        private void button3_Click(object sender, EventArgs e)
        {
            personalBindingSource.Filter = "fam='" + comboBox1.Text + "'";
        }

        private void button4_Click(object sender, EventArgs e)
        {
            personalBindingSource.Filter = "";
        }
        ...
        private Db_person personal;
        private void button6_Click(object sender, EventArgs e)
        {
            personal = new Db_person();
